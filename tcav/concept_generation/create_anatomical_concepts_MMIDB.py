...
def save_concept(DATA_PATH_CONCEPTS, raw, concept, patient, run, idx, code, NUMBER_CHANNELS=19, NUMBER_SAMPLES=1024):

    x = torch.zeros((1, NUMBER_CHANNELS, NUMBER_SAMPLES))
    x[:, :NUMBER_CHANNELS, :] = torch.from_numpy(raw.copy().get_data()[:, :NUMBER_SAMPLES].reshape(1, NUMBER_CHANNELS, NUMBER_SAMPLES))
    # x = min_max_normalize_dn3(x)
    # x[:,NUMBER_CHANNELS-1,:] = torch.ones((1, NUMBER_SAMPLES)) * -1  
    
    picklePath = DATA_PATH_CONCEPTS + concept + '/' + patient + run + '_' + idx + '_' + concept + '_' + code + '.pkl'
...
def define_concepts(DATA_PATH_RAW, DATA_PATH_CONCEPTS, activity_dict, patients, runs_to_use, bands_to_use, codes_to_use, label_names, WINDOW_LENGTH=4, NUMBER_CHANNELS=20, NUMBER_SAMPLES=1024, resting_state=True, sigma=True):
    ...
    for patient in tqdm(patients):
        ...
        if not resting_state:
            ...
            # TODO: Add std
            baseline_activity_std = np.array([activity_dict[band][patient][patient+baseline_run]['T0'].std(axis=0) for band in bands_to_use])
            baseline_activity_std = baseline_activity_std.reshape(number_bands, number_labels)
        ...
        for run in runs_to_use:
            ...
            if resting_state:
                ...
                baseline_activity_mean = baseline_activity_mean.reshape(number_bands, number_labels)
                # TODO: Add std
                baseline_activity_std = np.array([activity_dict[band][patient][patient+run]['T0'].std(axis=0) for band in bands_to_use])
                baseline_activity_std = baseline_activity_std.reshape(number_bands, number_labels)
            ...
            for code in codes_to_use:
                for raw_idx, raw in enumerate(annotation_dict[code]):
                    ...
                    save_concept(DATA_PATH_CONCEPTS, raw, concept, patient, run, str(raw_idx), NUMBER_CHANNELS=NUMBER_CHANNELS, NUMBER_SAMPLES=NUMBER_SAMPLES, code=code)
