...
def load_tuev_annotation(file_path, use_montage_20=False, remap_events=True):
    annot_path = file_path.split(".edf")[0] + ".rec"
    ...
    # make ch_idx and event int
    df["ch_idx"] = df["ch_idx"].astype(int)
    ...
    if remap_events:
        ...
        df.replace({"event": remap_dict}, inplace=True)
        # we get rid of the ch_idx because our windows are not channel specific
        df = df.drop(columns=['ch_idx']).drop_duplicates()
        df.reset_index(drop=True, inplace=True)
        # round values in start and end columns
        df['start'] = df['start'].round(4)
        df['end'] = df['end'].round(4)
    return df



def process_TUEV_edf(filepath, save_dir="/scratch/s194101/TUEV_processed/", verbose=False, use_montage_20=False):
    raw = mne.io.read_raw_edf(filepath, preload=True, verbose=False)
    annot_df = load_tuev_annotation(filepath, use_montage_20=use_montage_20)
    annot = mne.Annotations(onset=annot_df["start"].values,
                            duration=annot_df["end"].values - annot_df["start"].values,
                            description=annot_df["event"].values)
    raw.set_annotations(annot)

    subject_id = filepath.split("/")[-2]
    filename = os.path.basename(filepath)
    save_file_path = os.path.join(save_dir, subject_id, filename)
    if not os.path.exists(os.path.dirname(save_file_path)):
        os.makedirs(os.path.dirname(save_file_path))
                                  
    if verbose:
        print(f"Saving {filename} to {save_file_path}")
    raw.export(save_file_path, overwrite=True)
    return raw


def load_tusz_annotations(file_path, use_bi=True):
    if use_bi:
        annot_path = file_path.split(".edf")[0] + ".csv_bi"
    else:
        annot_path = file_path.split(".edf")[0] + ".csv"
    annot_df = pd.read_csv(annot_path, sep=",", skiprows=5)
    return annot_df

def process_TUSZ_edf(file_path, save_dir="/scratch/s194101/TUSZ_processed/", verbose=False):
    raw = mne.io.read_raw_edf(file_path, preload=True, verbose=False)
    annot_df = load_tusz_annotations(file_path)
    annot = mne.Annotations(onset=annot_df["start_time"].values,
                            duration=annot_df["stop_time"].values - annot_df["start_time"].values,
                            description=annot_df["label"].values)
    raw.set_annotations(annot)
    subject_id = file_path.split("/")[-4]
