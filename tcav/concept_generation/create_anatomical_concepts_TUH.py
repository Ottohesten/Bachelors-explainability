...
def get_activity_dict(ACTIVITY_PATH, SNR, info_dict, PARCELLATION='HCPMMP1_combined'):
    ...
    delta_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_1.0_4.0_' + str(SNR) + '_' + info_dict['1.0'][str(SNR)]['proj']['time'] + '_' + info_dict['1.0'][str(SNR)]['proj']['date'] + '_proj_tuh_selected.npy', allow_pickle=True).item()
    theta_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_4.0_8.0_' + str(SNR) + '_' + info_dict['4.0'][str(SNR)]['proj']['time'] + '_' + info_dict['4.0'][str(SNR)]['proj']['date'] + '_proj_tuh_selected.npy', allow_pickle=True).item()
    alpha_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_8.0_12.0_' + str(SNR) + '_' + info_dict['8.0'][str(SNR)]['proj']['time'] + '_' + info_dict['8.0'][str(SNR)]['proj']['date'] + '_proj_tuh_selected.npy', allow_pickle=True).item()
    beta_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_12.0_30.0_' + str(SNR) + '_' + info_dict['12.0'][str(SNR)]['proj']['time'] + '_' + info_dict['12.0'][str(SNR)]['proj']['date'] + '_proj_tuh_selected.npy', allow_pickle=True).item()
    gamma_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_30.0_70.0_' + str(SNR) + '_' + info_dict['30.0'][str(SNR)]['proj']['time'] + '_' + info_dict['30.0'][str(SNR)]['proj']['date'] + '_proj_tuh_selected.npy', allow_pickle=True).item()

    # delta_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_1.0_4.0_' + str(SNR) + '_' + info_dict['1.0'][str(SNR)]['no']['time'] + '_' + info_dict['1.0'][str(SNR)]['no']['date'] + '_no_proj_test.npy', allow_pickle=True).item()
    # theta_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_4.0_8.0_' + str(SNR) + '_' + info_dict['4.0'][str(SNR)]['no']['time'] + '_' + info_dict['4.0'][str(SNR)]['no']['date'] + '_no_proj_test.npy', allow_pickle=True).item()
    # alpha_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_8.0_12.0_' + str(SNR) + '_' + info_dict['8.0'][str(SNR)]['no']['time'] + '_' + info_dict['8.0'][str(SNR)]['no']['date'] + '_no_proj_test.npy', allow_pickle=True).item()
    # beta_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_12.0_30.0_' + str(SNR) + '_' + info_dict['12.0'][str(SNR)]['no']['time'] + '_' + info_dict['12.0'][str(SNR)]['no']['date'] + '_no_proj_test.npy', allow_pickle=True).item()
    # gamma_activity = np.load(ACTIVITY_PATH + PARCELLATION + '_30.0_70.0_' + str(SNR) + '_' + info_dict['30.0'][str(SNR)]['no']['time'] + '_' + info_dict['30.0'][str(SNR)]['no']['date'] + '_no_proj_test.npy', allow_pickle=True).item()

    # delta_activity = np.load(f"{ACTIVITY_PATH}/tuh_{PARCELLATION}_{SNR}_1.0_4.0_chop_parallel.npy", allow_pickle=True).item()
    # theta_activity = np.load(f"{ACTIVITY_PATH}/tuh_{PARCELLATION}_{SNR}_4.0_8.0_chop_parallel.npy", allow_pickle=True).item()
    # alpha_activity = np.load(f"{ACTIVITY_PATH}/tuh_{PARCELLATION}_{SNR}_8.0_12.0_chop_parallel.npy", allow_pickle=True).item()
    # beta_activity = np.load(f"{ACTIVITY_PATH}/tuh_{PARCELLATION}_{SNR}_12.0_30.0_chop_parallel.npy", allow_pickle=True).item()
    # gamma_activity = np.load(f"{ACTIVITY_PATH}/tuh_{PARCELLATION}_{SNR}_30.0_70.0_chop_parallel.npy", allow_pickle=True).item()
...
def save_concept(window, concept, DATA_PATH_CONCEPTS, edf_file, idx, NUMBER_CHANNELS=19, NUMBER_SAMPLES=1024):
    ...
    # picklePath = DATA_PATH_CONCEPTS + concept + '/' + edf_file.name[:-4] + '_' + str(idx) + '_' + concept + '.pkl'
...
def create_concepts(edf_files, DATA_PATH_RAW, DATA_PATH_CONCEPTS, activity_dict, bands, bands_to_use, standardize, sigma, absolute, NUMBER_CHANNELS=20, NUMBER_SAMPLES=1024):
    ...
    for edf_file in tqdm(edf_files, total=len(edf_files)): 
        ...
        for idx, annotation in enumerate(annotations):
            ...
            print(power.shape)
            print(baseline_power.shape)
            print(baseline_variance.shape)
...
if __name__ == '__main__':
    ...
    # ACTIVITY_PATH = '/home/s194101/TCAV-BENDR-own/tuh_HCPMMp1_combined'
    ACTIVITY_PATH = '/scratch/s194101/tuh_eeg_selected_processed_proj/'
    ...
    # activity_dict = get_activity_dict(ACTIVITY_PATH, SNR=SNR, info_dict=None)
    ...
    # DATA_PATH_RAW = '/scratch/s194101/tuh_eeg_random_100/'
    DATA_PATH_RAW = '/scratch/agjma/tuh_selected/'

    now = datetime.datetime.now()
    now_str = now.strftime("%H%M%S_%d%m%y")

    concept_folder = f'TUH_clean_{bands_naming}_{standardize}_{sigma}sigma_{absolute}abs_{str(SNR)}_{now_str}_test'
    DATA_PATH_CONCEPTS = f'/scratch/s194101/concepts/{concept_folder}/'
    create_concepts(edf_files, DATA_PATH_RAW, DATA_PATH_CONCEPTS, activity_dict, bands, bands_to_use, standardize, sigma, absolute, NUMBER_CHANNELS=19, NUMBER_SAMPLES=1024)