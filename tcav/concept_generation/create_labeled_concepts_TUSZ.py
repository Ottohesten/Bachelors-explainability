...
def read_tusz_edf(file_path, process=True, notch_filter=False):
...
def create_tusz_concepts(edf_files, save_dir, window_length):
    """
    ...
    for file_path in tqdm(edf_files):
        raw = read_tusz_edf(file_path)
        annotations = raw.annotations

        for idx, annotation in enumerate(annotations):
            onset, duration, description, _ = annotation.values()
            window = get_window(raw, annotation, window_length)
            if window is None or window.shape[0] != 19:
                continue
...
if __name__ == "__main__":
    TUEV_dir_path = "/scratch/s194101/TUSZ_processed"
    save_dir = "/scratch/s194101/concepts/TUSZ_2"
    ...
    create_tusz_concepts(edf_files, save_dir, 60.0)
